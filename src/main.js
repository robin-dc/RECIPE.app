        ...
        const categories = array.map(category => {
            return `<div class="flex flex-col rounded-2xl overflow-hidden shadow-2xl bg-white border border-gray-300">
                        ...
                        <div class="p-1 flex flex-col justify-between h-full">
                            <h3 class="text-[1.5rem] md:text-2xl font-bold text-black">${category.strCategory}</h3>
                            <p class="text-sm md:text-base text-gray-700">${shortenDescription(category.strCategoryDescription)}</p>
...
function renderMeals(array, category){
        const meals = array.map(meal => {
            return `<div class="flex flex-col rounded-2xl overflow-hidden shadow-2xl bg-white border border-gray-300 max-h-[21rem] md:max-h-[23rem]">
                        ...
                        <div class="p-1 flex flex-col justify-between h-full">
                            <h3 class="text-[1rem] md:text-[1.4rem] font-bold text-black">${shortenTitle(meal.strMeal)}</h3>
                            <div class="flex flex-col md:flex-row justify-center md:justify-between items-center mt-[0.5rem] md:mt-1 gap-[0.3rem] md:gap-0">
                                <small class="block text-base text-gray-500"><i class="fa-regular fa-clock mr-[0.4rem]"></i>30 Minutes</small>
                                <button class="w-full md:w-fit rounded-lg text-gray-600 border-2 border-gray-600 bg-white py-[0.5rem] px-1 font-bold" id="view-recipe-btn" data-id="${meal.strMeal}">View Recipe</button>
                            </div>
                        </div>
                    </div>`
        }).join("")
...
async function getRecipeData(meal) {
    try{
        ...
        foodContainer.innerHTML = `<button class="rounded-full bg-white shadow-lg cursor-pointer border border-gray-200 w-3 h-3 absolute top-0 left-1 md:left-0 flex justify-center items-center" id="back-meals-btn"><i class="fa-solid fa-arrow-left text-black"></i></button>
        <div class="col-span-4 flex flex-col gap-1 md:gap-3">
            <div class="rounded-2xl shadow-xl pt-2 p-1 md:p-3 leading-[3rem] border border-gray-200" >
                <h1 class="text-3xl md:text-4xl font-bold text-lightBrown">${object.strMeal}</h1>
                <p class="text-[1.4rem] md:text-2xl leading-[2rem] md:leading-[3rem] tracking-wide text-navyBlue">
                    Meal originated from ${object.strArea} folks
                </p>
                <div class="w-full h-[25rem] mt-2 mb-2 md:mb-4 bg-black rounded-tr-[5rem] rounded-bl-[5rem] rounded-3xl overflow-hidden">
                    <img src="${object.strMealThumb}" class="w-full h-full object-cover opacity-80" alt="">
                </div>
                <hr>
                <div class="my-2 md:my-4">
                    <h2 class="text-center text-3xl font-bold">Ingredients</h2>
                    <ul class="w-3/5 mx-auto mt-3 flex flex-col md:flex-row gap-[0.5rem] md:gap-2 md:flex-wrap list-disc text-xl">
                        ${renderIngredients(object)}
                    </ul>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-[0.5rem] md:gap-2 mt-3">
                <h2 class="text-center text-3xl font-bold col-span-1 md:col-span-4 mb-1">Instructions</h2>
                    ${renderInstructions(object.strInstructions)}
            </div>
            <div class="mt-3 p-0 md:p-2">
                <h2 class="text-center text-3xl font-bold">Video Tutorial</h2>
                <iframe class="w-full h-[23rem] md:h-[35rem] my-2 md:my-3 rounded-2xl" src="https://www.youtube.com/embed/${getEmbeddedCode(object.strYoutube)}" frameborder="0" allowfullscreen></iframe>
            </div>
        </div>`
...
function renderIngredients(data){
    ...
    for(item in data){
        if(item.startsWith("strIngredient") && data[item]){
            ingredients.push(`<li><strong class="text-red-700">${data['strMeasure'+ count]}</strong> ${data[item]}</li>`)
            count++;
        }
    }
    ...
    for(let i = 0; i < data.length; i++){
        if(i > 5 && !isNaN(Number(data[i])) && data[i+1] === "."){
            ...
            if(Number(data[i]) !== (steps.length+2)){
                ...
                steps.push(`<div class="rounded-2xl shadow-xl p-2 bg-wood text-white h-fit md:h-full w-full">
                            <h4 class="text-[1.4rem] md:text-2xl font-bold mb-1">Step ${steps.length+1}</h4>
                            <p>${str}</p>
                        </div>`)
                str = ""
                period = 0;
...
function foodError(){
    foodContainer.innerHTML = `<p class="fixed top-[40%] left-[50%] right-[50%] translate-x-[-50%] text-gray-400 text-[1.6rem] md:text-2xl font-semibold block w-full text-center">The food you are craving cannot be found.</p>`
}
function foodMissing(){
    foodContainer.innerHTML = `<p class="fixed top-[40%] left-[50%] right-[50%] translate-x-[-50%] text-gray-400 text-[1.6rem] md:text-2xl font-semibold block w-full text-center">The search bar is empty.</p>`
}
function favoriteError(){
    foodContainer.innerHTML = `<p class="fixed top-[40%] left-[50%] right-[50%] translate-x-[-50%] text-gray-400 text-[1.6rem] md:text-2xl font-semibold block w-full text-center">Your favorite foods is empty.</p>`
}
function backBtn(category){
    return `<button class="rounded-full bg-white shadow-lg cursor-pointer border border-gray-200 px-1 h-3 absolute top-0 md:left-0 left-1 flex justify-center items-center gap-[0.5rem]" id="back-category-btn"><i class="fa-solid fa-arrow-left text-black"></i><span class="font-semibold">${category} Category</span></button>`
}
